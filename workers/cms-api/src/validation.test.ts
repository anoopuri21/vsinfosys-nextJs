...
const validInput = {
  ...
  summary:
    "A conversion-focused commerce experience designed for a clear buying journey.",
  seoTitle: "E-commerce development | VS Infosys",
  seoDescription:
    "Conversion-focused e-commerce development for businesses that need a clearer, faster buying journey.",
...
test("rejects unknown CMS section types", () => {
  assert.throws(
    () =>
      assertContentInput({
        ...validInput,
        body: { sections: [{ type: "scriptInjection" }] },
      }),
    ApiValidationError,
  );
});
...
test("rejects unsafe canonical paths and external CTAs", () => {
  ...
  assert.throws(
    () =>
      assertContentInput({
        ...validInput,
        body: { sections: [{ type: "cta", ctaUrl: "https://example.com/" }] },
      }),
    ApiValidationError,
  );
});
...
test("rejects malformed root body and unsafe slugs", () => {
  assert.throws(
    () => assertContentInput({ ...validInput, slug: "Unsafe Slug" }),
    ApiValidationError,
  );
  assert.throws(
    () => assertContentInput({ ...validInput, body: [] }),
    ApiValidationError,
  );
});
